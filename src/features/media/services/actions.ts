...
export async function addToUserSavedMedia(
  ...
  try {
    ...
    return {
      success: false,
      message: (err as Error).message || `Something went wrong! Could not add to your ${listType}.`,
    };
  }
}

export async function updateUserRating(tmbdId: number, newRating: number) {
  try {
    if (newRating <= 0) {
...
export async function removeUserSavedMedia(tmbdId: number) {
  try {
    // just update user rating without needing to check if tmbdId exists since if does't no row will match
    const supabase = await createClient();
    const {
